...
export function AuthStatusSection({jwAuth, menuItemStyle}: Props) {
    const navigation = useNavigation();
    const {theme} = useTheme();

    const items = [
        { label: "教务系统", icon: "school", auth: jwAuth, screen: "jwAccount" },
        { label: "统一认证", icon: "shield-account", auth: null, screen: "authAccount" },
        { label: "考勤系统", icon: "clock-check", auth: null, screen: "attendanceAccount" },
    ];

    return (
        <View>
            <Text style={{fontSize: 12, color: theme.colors.grey3, paddingHorizontal: 8, paddingBottom: 4}}>账号状态</Text>
            {items.map(item => (
                <Pressable key={item.label} style={menuItemStyle}
                    ...
                    <Flex align="center" gap={16} style={{flex: 1}}>
                        ...
                        <Icon
                            ...
                            size={20}
                            color={item.auth?.status === AuthStateMap.Authenticated ? theme.colors.success : theme.colors.error}
                        />
                    </Flex>
                </Pressable>
            ))}
